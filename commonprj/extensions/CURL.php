...
class CURL
{
	...
	{
		Yii::trace('cURL Class Initialized');

		if ( ! $this->is_enabled())
		{
			Yii::error('cURL Class - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.', 'curl-error');
		}
	...
	public function head($params, $options = array())
	{
		// If its an array (instead of a query string) then format it correctly
		if (is_array($params))
		{
